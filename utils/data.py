...
def remove_empty(directory: str, n_midi_files: int=0):
    '''
    Remove directories where all midi files are empty
    '''
    if n_midi_files > 0:
        n_rows = 0
        files = []
...
def pitch_range(dfs: list[pd.DataFrame]):
    ...
    ax.scatter(range(len(pitch_mins)), (pitch_maxs+pitch_mins) / 2, color='red', linewidths=1.0);
    plt.show()
...
class NoteDurationDataset(Dataset):
    ...
    def __init__(self, dfs: list[pd.DataFrame], sample_len: int, scale: float=1., stride: int=1):
        ...
        assert dfs[0].columns.tolist() == ['Start', 'End', 'Pitch'], 'Columns must be in order: Start, End, Pitch'
