    ...
    def recombine(self, out_device=None):
        ...
        if out_device is None:
            if self.dim is None:
                ...
                device = self.split_tensors[0].device
                return torch.cat([tensor.to(device) if tensor.device != device else tensor for tensor in self.split_tensors], dim=self.dim)
        else:
            if self.dim is None:
                if self.split_tensors[0].device == out_device:
                    return self.split_tensors[0]
                else:
                    return self.split_tensors[0].to(out_device)
            else:
                return torch.cat([tensor.to(out_device) if tensor.device != out_device else tensor for tensor in self.split_tensors], dim=self.dim)
    ...
    def cumsum(self, dim=0, inplace=False):
        ...
        if dim == self.dim:
            ...
            for tensor in self.split_tensors[1:]:
                ...
                if prev_sum.device != tensor.device:
    ...
    def _sum(self, dim=0, inplace=False):
        ...
        if dim == self.dim:
            ...
            for tensor in sum_tensors[1:]:
                sum_tensor += tensor.to(device) if tensor.device != device else tensor
            ...
            if inplace:
                ...
                return SplitTensor([sum_tensor], [device.index], dim=None)
        else:
            if inplace:
                if self.dim:
                    if dim < self.dim:
