...
from .. import utils
from ..utils import RandomStateOrSeed
...
class LowRankFeatures(Kernel, TransformerMixin, metaclass=ABCMeta):
    ...
    Any classes deriving from this at the very least should implement the following methods:
        _make_feature_components(X : torch.Tensor): this (void) method should initialize dependent variables (except n_features_)
        _compute_features(X : torch.Tensor): this method should compute and return the finite dimensional embedding for input data X
    ...
    def _check_n_features(self, X : torch.Tensor, reset : bool = False) -> None:
        ...
        Args:
            X (torch.Tensor): Input data as a NumPy or CuPy array.
    ...
    def _validate_data(self, X : torch.Tensor, reset : bool = False) -> torch.Tensor:
        ...
        Returns:
            torch.Tensor: returns the input data as a NumPy or CuPy array without any changes.
        """
    ...
    def fit(self, X: torch.Tensor, y : Optional[torch.Tensor] = None) -> LowRankFeatures:
        """Validates the input data and initializes all dependent variables.

        Args:
            X (torch.Tensor): Input data as a NumPy or CuPy array.
            y (Optional[torch.Tensor], optional): Only provided for consistency with the sklearn API. Defaults to None.
    ...
    def _compute_features(self, X : torch.Tensor) -> torch.Tensor:
        ...
        Args:
            X (torch.Tensor): Input data as a CuPy array.

        Returns:
            torch.Tensor: A feature set as a CuPy array.
        """
        pass

    def transform(self, X : torch.Tensor, return_on_gpu : bool = False) -> torch.Tensor:
        ...
        Returns:
            X_feat: (torch.Tensor) A feature set as a NumPy or CuPy array.
        """
    ...
    def _K(self, X : torch.Tensor, Y : Optional[torch.Tensor] = None) -> torch.Tensor:
        ...
        Args:
            X (torch.Tensor): First input data CuPy array.
            Y (Optional[torch.Tensor], optional): Second input data CuPy array. Defaults to None.

        Returns:
            torch.Tensor: The kernel matrix between X and Y or X and X as a CuPy array.
        """
        X_feat = self._compute_features(X)
    ...
    def _Kdiag(self, X : torch.Tensor) -> torch.Tensor:
        ...
        Args:
            X (torch.Tensor): Input data CuPy array.

        Returns:
            torch.Tensor: The diagonal entries of the kernel matrix between X and X as a CuPy array.
        """
        X_feat = self._compute_features(X)
        return utils.squared_norm(X_feat, axis=-1)

    def __call__(self, X : torch.Tensor, Y : Optional[torch.Tensor] = None, diag : bool = False, return_on_gpu : bool = False) -> torch.Tensor:
        ...
        Args:
            X (torch.Tensor): First input data as a NumPy or CuPy array.
            Y (Optional[torch.Tensor], optional): Second input data as a NumPy or CuPy array. Defaults to None.
            diag (bool, optional): Whether to compute only the diagonal entries (only if Y is None). Defaults to False.
            return_on_gpu (bool, optional): Whether to return the output on GPU (as CuPy) or on CPU (as NumPy).

        Returns:
            torch.Tensor: A kernel matrix or the diagonal entries of a kernel matrix as a NumPy or CuPy array.
        """
        check_is_fitted(self)
        X = self._validate_data(X)
        if Y is not None:
...
class NystroemFeatures(LowRankFeatures):
    ...
    def _make_feature_components(self, X : torch.Tensor) -> None:
        ...
        #     basis_inds = torch.asnumpy(basis_inds) # doing this seems to be the most efficient
        # basis = torch.asarray(X[basis_inds])
        basis = X[basis_inds]
    ...
    def _compute_features(self, X : torch.Tensor) -> torch.Tensor:
        ...
        Returns:
            torch.Tensor: Nystroem features as a CuPy array.
        """
...
class RBFFourierFeatures(LowRankFeatures):
    ...
    def __init__(self, sigma : float = 1., n_components : int = 100, random_state : Optional[RandomStateOrSeed] = None) -> None:
        """Initializer for the RBFFourierFeatures class.

        Args:
            sigma (float, optional): The sigma of the inputs (divides the input, x' = x / l for input x). Defaults to 1..
            n_components (int, optional): The number of feature components to use in the embedding. Defaults to 100.
            random_state (Optional[RandomStateOrSeed], optional): Random state or seed value for reproducibility. Defaults to None.
        """
        super().__init__(n_components=n_components, random_state=random_state)
    ...
    def _make_feature_components(self, X : torch.Tensor) -> None:
        ...
        self.n_components_ = self.n_components
        random_state = utils.check_random_state(self.random_state)
        self.random_weights_ = (torch.sqrt(torch.tensor(2 / self.sigma**2, dtype=dtype)) *
                                torch.randn(size=(self.n_features_, self.n_components_), generator=random_state, device=X.device, dtype=dtype))
        self.random_offset_ = torch.rand(self.n_components_, generator=random_state, device=X.device, dtype=dtype) * 2 * np.pi
    ...
    def _compute_features(self, X : torch.Tensor) -> torch.Tensor:
        ...
        torch.cos(projection, projection)
        projection *=  torch.sqrt(2.) / torch.sqrt(self.n_components)
