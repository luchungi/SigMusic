...
def sig_kernel_batch_varpar(floating[:,:,:] G_static, bint _naive_solver=False):
	...
	if floating is float:
		K = np.zeros((A,M+1,N+1), dtype=np.float32)
	elif floating is double:
		K = np.zeros((A,M+1,N+1), dtype=np.float64)
...
def sig_kernel_Gram_varpar(floating[:,:,:,:] G_static, bint sym=False, bint _naive_solver=False):
	...
	if floating is float:
		K = np.zeros((A,B,M+1,N+1), dtype=np.float32)
	elif floating is double:
		K = np.zeros((A,B,M+1,N+1), dtype=np.float64)
